...
def preproces_missing_rows_roi(data_type='pretrain', custom_file_path='', custom_img_path='', batch_size=64, gpu_index=0):
    device = torch.device(f"cuda:{gpu_index}" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")
    ...
    if data_type == 'pretrain':
        ...
        if not os.path.exists(data_path):
            raise FileNotFoundError(f'Custom file path {data_path} does not exist')
    else:
        raise ValueError(
            'Invalid data_type: {} or custom_file_path not defined. '
            'Please provide either "pretrain", "finetune", or a "custom_file_path".'.format(data_type)
    ...
    images_root = f'data/llava-{data_type}/images' if not custom_img_path else custom_img_path
    json_data_path = data_path.replace('.json', f'_roi_fixed.json')
        
    df = pd.read_json(data_path)
    
    roi_pipeline = ROISelection(device=device)
    
    # get df with rows where any of 'ram_tags', 'select_tags' or 'bboxes' is empty
    subset_df = df[
        df['ram_tags'].apply(lambda x: not bool(x)) | # Check if empty
        df['select_tags'].apply(lambda x: not bool(x)) |   # Check if empty
        df['bboxes'].apply(lambda x: not bool(x))  # Check if empty
        # 'image' should not be none
        & (df['image'].apply(lambda x: not bool(x)))
    ]
    # print the number of missing rows
    print(f'Number of missing rows: {len(subset_df)}')
...
if __name__ == '__main__':
    fire.Fire({
        ...
        'roi_fix': preproces_missing_rows_roi,
    })
