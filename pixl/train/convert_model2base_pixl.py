...
def train():
    ...
    if model_args.vision_tower is not None:
        # pixl config: load llm config from pretrained model
        if "phi3" in model_args.model_name_or_path or "phi-3" in model_args.model_name_or_path:
            config = PIXLPhi3Config.from_pretrained(model_args.model_name_or_path, trust_remote_code=True)
        ...
        # pixl vision config
        pixl_vis_config = PIXLVisionConfig(**vision_config.to_dict())

        # pixl config: update pixl config using pixl vision config
        config.vision_config["vision_tower"] = pixl_vis_config.to_dict()
        ...
        config.vision_config["mm_projector"]["mm_hidden_size"] = pixl_vis_config.hidden_size
        config.vision_config["mm_projector"]["hidden_size"] = config.hidden_size

        # load pixl, this would only load the slm part of the model, the rest of the model will be initialized later
        if "phi3" in model_args.model_name_or_path or "phi-3" in model_args.model_name_or_path:
        ...
        raise ValueError("PIXL only support multimodal models. Please specify vision_tower in model_args.")
    ...
    print(f"Using conversation format: {conversation_lib.default_conversation}")
